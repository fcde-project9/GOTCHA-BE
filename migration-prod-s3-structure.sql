-- ========================================
-- S3 폴더 구조 변경: Prod Database (postgres)
-- ========================================
-- 목적: S3 URL 경로에 prod/ prefix 추가
-- 실행 환경: DBeaver에서 postgres database 연결 후 실행
-- 작업일: 2026-01-20
-- ⚠️ 주의: Dev 환경 테스트 완료 후 실행할 것!
-- ========================================
...
SELECT
    ...
    REPLACE(REPLACE(profile_image_url, '/profiles/', '/prod/profiles/'), '/defaults/', '/prod/defaults/') AS new_url
FROM users
WHERE profile_image_url IS NOT NULL
...
SELECT
    ...
    REPLACE(REPLACE(main_image_url, '/shops/', '/prod/shops/'), '/defaults/', '/prod/defaults/') AS new_url
FROM shops
WHERE main_image_url IS NOT NULL
...
SELECT
    ...
    REPLACE(image_url, '/reviews/', '/prod/reviews/') AS new_url
FROM review_images
WHERE image_url IS NOT NULL
...
-- users 테이블 업데이트
UPDATE users
SET profile_image_url = REPLACE(profile_image_url, '/profiles/', '/prod/profiles/')
WHERE profile_image_url LIKE '%/profiles/%';

UPDATE users
SET profile_image_url = REPLACE(profile_image_url, '/defaults/', '/prod/defaults/')
WHERE profile_image_url LIKE '%/defaults/%';

-- shops 테이블 업데이트
UPDATE shops
SET main_image_url = REPLACE(main_image_url, '/shops/', '/prod/shops/')
WHERE main_image_url LIKE '%/shops/%';

UPDATE shops
SET main_image_url = REPLACE(main_image_url, '/defaults/', '/prod/defaults/')
WHERE main_image_url LIKE '%/defaults/%';

-- review_images 테이블 업데이트
UPDATE review_images
SET image_url = REPLACE(image_url, '/reviews/', '/prod/reviews/')
WHERE image_url LIKE '%/reviews/%';

-- 변경 사항 확인
SELECT 'Updated users:' AS info, COUNT(*) AS count FROM users WHERE profile_image_url LIKE '%/prod/%'
UNION ALL
SELECT 'Updated shops:' AS info, COUNT(*) AS count FROM shops WHERE main_image_url LIKE '%/prod/%'
UNION ALL
SELECT 'Updated review_images:' AS info, COUNT(*) AS count FROM review_images WHERE image_url LIKE '%/prod/%';
