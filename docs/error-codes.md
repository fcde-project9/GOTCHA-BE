...
| A013 | 400 | 유효하지 않거나 만료된 인증 코드입니다 | 잘못된 인증 코드 |
| A014 | 403 | 정지된 사용자입니다 | 정지 사용자 로그인/API 접근 시도 (suspended_until 정보 포함) |
| A015 | 403 | 차단된 사용자입니다 | 영구 차단 사용자 로그인/API 접근 시도 |
...
| U006 | 400 | 허용되지 않는 정지 기간입니다 | 허용된 시간 외 정지 기간 지정 |
...
| 코드 | HTTP | 메시지 | 설명 |
|------|------|--------|------|
| BK001 | 400 | 본인을 차단할 수 없습니다 | 자기 자신 차단 시도 |
| BK002 | 409 | 이미 차단한 사용자입니다 | 중복 차단 |
| BK003 | 404 | 차단 정보를 찾을 수 없습니다 | 차단하지 않은 사용자 해제 시도 |
| BK004 | 400 | 차단할 수 없는 사용자입니다 | 탈퇴/정지/차단된 사용자 차단 시도 |
