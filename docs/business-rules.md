...
```java
// openTime 형식: JSON 문자열 (예: {"Mon":"10:00-22:00","Tue":null,"Wed":"10:00-22:00"})
...
2. 오늘 요일의 영업시간이 null이면 → openStatus = "휴무"
3. 오늘 요일의 영업시간 형식이 잘못되면 → openStatus = ""
4. 현재 시간이 영업시간 범위 내인지 확인:
   - 범위 내면 → openStatus = "영업 중"
   - 범위 밖이면 → openStatus = "영업 종료"
5. 자정을 넘어가는 영업시간도 지원 (예: "22:00-02:00")
